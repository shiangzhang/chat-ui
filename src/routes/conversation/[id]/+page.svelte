...
async function writeMessage({
	...
	try {
		...
		if (isContinue && messageId) {
			if (
				(messages.find((msg) => msg.id === messageId)?.children?.length ?? 0) >
				0
			) {
		...
		for await (const update of messageUpdatesIterator) {
			...
			if (
				...
				const convInData = data.conversations.find(
					({ id }) => id === $page.params.id,
				);
				if (convInData) {
...
async function voteMessage(score: Message["score"], messageId: string) {
	...
	try {
		await fetch(
			`${base}/conversation/${conversationId}/message/${messageId}/vote`,
			{
				method: "POST",
...
async function onRetry(
	event: CustomEvent<{ id: Message["id"]; content?: string }>,
) {
	if (!data.shared) {
		await writeMessage({
